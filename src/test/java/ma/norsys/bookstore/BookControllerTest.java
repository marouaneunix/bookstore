...
public class BookControllerTest {
    ...
    @BeforeEach
    void init() {
        firstBook = new Book(1, "123-5444", "science_de_vie", "John", "science", "science");
        secondBook = new Book(2, "123679", "science_de_terre", "Been", "science", "science de terre");
    }
    ...
    void givenExistentBookName_whenGetByName_thenStatus200() throws Exception {
        when(bookService.getBookByName(firstBook.getName())).thenReturn(firstBook);
    ...
    when(bookService.getBookByName("hero")).thenThrow(new BookNotFoundException("Book Not Found"));
    ...
    void givenExistentBookCategory_whenGetByCategory_thenStatus200() throws Exception {
        ...
        this.mockMvc.perform(MockMvcRequestBuilders.get("/api/books/category/{category}", firstBook.getCategory())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    when(bookService.getBookByCategory("drama")).thenThrow(new BookNotFoundException("Book Not Found"));
